...
const Form = () => {
  ...
  const [formErrors, setFormErrors] = useState([
    ...
    { id: 4, message: "ERROR: Password must be more than 6 characters.", active: false },
  ]);
  ...
  return (
    <>
      <form onSubmit={handleSubmit} style={formStyle} noValidate>
        ...
        <br />
        <Password value={formValues.password} onChange={handleChange} className={styles.form} />
        {formErrors.find((error) => error.id === 4 && error.active) && (
          <div style={errorStyle}>
            <p>{formErrors.find((error) => error.id === 4).message}</p>
          </div>
        )}
        <br />
